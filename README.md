...
其他裝置則要等下一次輪詢才會看到，因此**最多約 4～8 秒的延遲**。

自己操作的畫面是立即反應的（本機狀態先更新），延遲只發生在**跨裝置**的同步。

> 若日後需要真正的即時更新，可將 `get()` 改為 Firebase 的 `onValue()` 監聽，
> 資料一變動即主動推送。目前的輪詢方式對評分場景已足夠。
