...
| 我要… | 怎麼做 |
|---|---|
| 開始一場比賽 | 總控台 → 名單匯入 → 場地與裁判 → 項目控制（把項目排進場地佇列，按「開始」） |
...
- **總控台** — 需密碼。名單匯入、場地與裁判、項目控制（場地排程看板）、即時監看、排名結果、最佳運動員
...
看板分成「資料庫」欄與每個場地一欄。**資料庫**放所有還沒排入場地的項目，保留快速搜尋。

**排項目**

- **手機**：點一下項目選取它 → 點頂部的目標場地分頁（會顯示「↓放這」）放入。每張卡片也有上下箭頭調順序、「移至…」下拉、撤銷鍵拉回資料庫
- **桌面**（寬度 ≥900px 且非觸控）：可直接用卡片左側的把手拖放，跨欄與同欄排序都可以。點選操作同樣可用

**佇列規則**

- 每個場地**同時只跑首位一個項目**。排進場地的項目預設是「等候中」，裁判看不到
- 首位按「**開始**」才會開啟，裁判端即時跳到該項目，不需重新整理
- 正在評分的項目**不能被插隊**（拖放或移動到第 0 位會自動退到第 1 位）。要換人先按撤銷
- 「**自動**」開關打開後，該場地的分數與裁判長加分收齊約 2 秒後，會自動把目前項目標記為已完成並開啟下一個，裁判畫面跟著跳
  - 注意：自動接續由總控台執行，**需保持總控台開著**（任何一台裝置都可以，分頁可以隨意切到即時監看／排名結果，不影響）。整個關掉才會停，重新開啟後會補跑
- 欄位標題的跳過鍵可手動跳到下一項；若尚未收齊分數會先跳出確認
- 已完成的項目收在欄位底部的「已完成」區，按「**重新開啟**」可以拉回首位改分
...
| 管理端排程看板 → 另一台管理裝置 | 兩人同時操作也會互相看到 |
| 裁判送出分數 → 管理端「即時監看」 | 需**選定該項目**才即時 |
| 裁判送出分數 → 項目控制的首位進度 | 只監看每個場地的首位項目 |
...
**技術棧**：Create React App · Firebase Realtime Database · lucide-react · @dnd-kit（桌面拖放）
...
| `groups-meta` | 項目（分組）狀態：所屬場地、是否開啟 |
| `queues` | 各場地的項目順序、已完成清單、自動接續開關 |
